...
def findPasta(image):
    ...
    mask = cv2.inRange(blur, (20, 105, 145), (179, 255, 255)) 
    ...
    for i in range(len(contours)):
        ...
        if(area > 300 and area < 3000):
            pastaPiece = Food.Pasta(image=cropped_image)
...
for root, dirs, files in os.walk(path):
    ...
    for filename in files:
        ...
        pasta.extend(findPasta(image))

    for p in range(len(pasta)):
        print(pasta[p].area)
        cv2.imwrite("D:/Cook3r/Cook3r/Pasta_detect/Images/"+str(p)+".jpg", pasta[p].image)